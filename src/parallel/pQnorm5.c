...
*	@returns >0 if everything was fine <0 if there was an error
   ...
   pQnorm5 : to share a file we must open it before the parallel section 
             and declare the Handle as shared
*
...
#pragma omp parallel shared(nG, nE,mIndex,dataIn, dIndex, AvG, fList, p,fI,fOut) private(i,j,k,tid,nP1, dataOut, From, To, Range)
...
*/
   ...
   fprintf(stderr, "P41 tid=%d 2da parte\n",tid); fflush(stderr);
   ...
   for (i=0;i<nE;i++) {
        ...
        if (p->MemIndex) { // in memory - full
          ...
          pos = fList[i].pos;
          fseek(fI, posbasei*pos, SEEK_SET);
   ...
   fprintf(stderr, "P42 tid=%d ha grabado el exp %d en posic=%ld\n",tid,i,pos*posbase); fflush(stderr);
